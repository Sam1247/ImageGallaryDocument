...
class ImageGallaryCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout, UICollectionViewDropDelegate, UICollectionViewDragDelegate {
    
    // MARK: - Model
    
    var imageGallary: ImageGallary? = ImageGallary() {
        didSet {
            collectionView.reloadData()
            print("imageGallary has been set")
        }
    }

    var document: ImageGallaryDocument?
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        document?.open { success in
            if success {
                ...
                if self.document?.imageGallary != nil {
                    self.imageGallary = self.document?.imageGallary
                }
                print("document oppeded")
            }
        }
    }
    
    @IBAction func save(_ sender: Any) {
        document?.imageGallary = imageGallary
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        let cellHeight = CGFloat(imageGallary!.imagesData[indexPath.row].aspectRatio) * cellWidth
                            ...
                            DispatchQueue.main.async {
                                if let imageData = data, let image = UIImage(data: imageData) {
                                    ...
                                    placeholderContext.commitInsertion(dataSourceUpdates: { insertionIndexPath in
                                        self.imageGallary!.imagesData.insert(ImageData(url: url, aspectRatio: Double(aspectRatio)), at: insertionIndexPath.item)
                                    })
                                } else {
